...
class ViT_CLIP(nn.Module):
    ...
    def forward(self, x: torch.Tensor, output_format: str = "flatten") -> torch.Tensor:
        """
        Args:
            x: (B, T, C, H, W)
            output_format: 'swin' | 'flatten'
        """
        B, T, C, H, W = x.shape
        ...
        # ====================== 输出格式 ======================
        if output_format == "swin":
            # Swin 格式：(B, 768, 4, 7, 7)
            spatial_feat = x[:, 1:].reshape(B, self.num_frames, 768, 7, 7)
            spatial_feat = spatial_feat[:, ::2, :, :, :]
            return spatial_feat.permute(0, 2, 1, 3, 4).contiguous()
