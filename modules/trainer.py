        ...
        for epoch in range(0, cfg.train.epochs):
            ...
            for step, batch in enumerate(pbar):
                ...
                with self.accelerator.autocast():
                    p_v, p_o, p_pred = self.model(batch_img)
                    # component loss
                ...
                if accelerator.is_main_process:
                    ...
                    self.writer.add_scalar("train/loss_verb", loss_verb.item(), epoch * len(self.train_dataloader) + step)
                    self.writer.add_scalar("train/loss_obj", loss_obj.item(), epoch * len(self.train_dataloader) + step)
                    self.writer.add_scalar("train/loss_comp", loss_com.item(), epoch * len(self.train_dataloader) + step)
                    self.writer.add_scalar("train/lr", self.optimizer.param_groups[0]["lr"], epoch * len(self.train_dataloader) + step)
